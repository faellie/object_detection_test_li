#from https://github.com/julianklumpers/slice_image_with_annotations
import cv2
import os
...
def slice(image, xml, size=(4, 3), path='', suffix=''):
...
def main(_):
    xml_list = []
    for xml_file in glob.glob('/opt/TF/test/t1/' + '/*.xml'):
        xmlTree = ET.parse(xml_file)
        root = xmlTree.getroot()
        imageFile = root.find('filename').text
        image = misc.imread(os.path.join('/opt/TF/test/t1', imageFile))
        slice(image, xmlTree, path='/opt/TF/test/t1/out', suffix=imageFile[:-4])
